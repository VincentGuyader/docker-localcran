...
<CRANDORE_REPOS_PATH>/
  ...
  src/contrib/                                         ← .tar.gz + PACKAGES* (source mode only)
```

**Source mode** (`os: source`) produces a universal, platform- and R-version-independent
repository at `<CRANDORE_REPOS_PATH>/src/contrib/`. It is useful as a fallback mirror
that any R installation can point to, or as a rolling "latest" tap decoupled from
a specific distro / R release.
...
# Source (universal — any OS, any R version)
options(repos = c(CRAN = "http://your-server/"))
```
...
### Source-only (universal) repo

A single repo of source tarballs, usable from any OS and any R version.
No `distro`, `arch` or `r_version` required.
...
Output: `<CRANDORE_REPOS_PATH>/src/contrib/` with one `.tar.gz` per package
plus `PACKAGES*` files. Point R at the repo root:
...
|----------|-------------|---------|
| `CRANDORE_OS` | Target: `linux`, `windows`, or `source` (universal, platform-independent) | Current OS |
